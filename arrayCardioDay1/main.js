  ...
  const inventorsBirthdate = inventors.map(inventor => inventor.year);
  const numberSorter = (a, b) => b - a;
  console.table(inventorsBirthdate.sort(numberSorter));
  ...
  const boulevards = ['Boulevards de Paris', 'City walls de Paris', 'Thiers wall', 'Wall de Charles V',
    'Wall de Philip II Augustus', 'City gates de Paris', 'Haussmann\'s renovation de Paris',
    'Boulevards de the Marshals', 'Boulevard Auguste-Blanqui', 'Boulevard Barbès', 
    'Boulevard Beaumarchais', 'Boulevard de l\'Amiral-Bruix', 'Boulevard Mortier',
    'Boulevard Poniatowski', 'Boulevard des Capucines', 'Boulevard de la Chapelle', 
    ...
    'Boulevard Lefebvre', 'Boulevard de la Madeleine', 'Boulevard de Magenta',
    'Boulevard Montmartre', 'Boulevard du Montparnasse','Boulevard Raspail'
  ];

  const de = boulevards.map((item) => item)
    .filter((streetName) => streetName.includes('de '));
    
  // 7. sort Exercise
  // Sort the people alphabetically by last name
  const alphaNames = people.sort((lastOne, nextOne) => {
